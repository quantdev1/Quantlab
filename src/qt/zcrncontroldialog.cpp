...
#include "ui_zqntcontroldialog.h"
...
std::list<std::string> ZQntControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZQntControlDialog::listMints;

ZQntControlDialog::ZQntControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZQntControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZQntControlDialog::~ZQntControlDialog()
{
    delete ui;
}

void ZQntControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZQntControlDialog::updateList()
{
...
void ZQntControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZQntControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZQnt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZQntControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZQntControlDialog::GetSelectedMints()
{
...
void ZQntControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
