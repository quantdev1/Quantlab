...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::QNT);
    ...
    if (!settings.contains("nAnonymizeQuantlabAmount"))
        settings.setValue("nAnonymizeQuantlabAmount", 1000);

    nAnonymizeQuantlabAmount = settings.value("nAnonymizeQuantlabAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeQuantlabAmount"))
        SoftSetArg("-anonymizequantlabamount", settings.value("nAnonymizeQuantlabAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in quantlab.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeQuantlabAmount:
            return QVariant(nAnonymizeQuantlabAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeQuantlabAmount:
            nAnonymizeQuantlabAmount = value.toInt();
            settings.setValue("nAnonymizeQuantlabAmount", nAnonymizeQuantlabAmount);
            emit anonymizeQuantlabAmountChanged(nAnonymizeQuantlabAmount);
            break;
        case CoinControlFeatures:
