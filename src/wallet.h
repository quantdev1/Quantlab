...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 QNT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zQNT send
enum ZerocoinSpendStatus {
    ZQNT_SPEND_OKAY = 0,                            // No error
    ZQNT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZQNT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZQNT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZQNT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZQNT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZQNT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZQNT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZQNT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZQNT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZQNT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZQNT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZQNT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZQNT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZQNT_SPENT_USED_ZQNT = 14,                      // Coin has already been spend
    ZQNT_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZQntAutoBackups(bool fEnabled)
    {
    ...
    /** zQNT reset */
    boost::signals2::signal<void()> NotifyzQNTReset;
};
